...
from ccxtbt.utils import get_order_entry_price_and_queue, get_time_diff

from check_in_gating_tests.common.test__helper import ut__construct_standalone_account_or_store, \
    ut__construct_standalone_exchange, ut__construct_standalone_instrument, ut_get_valid_market_types
...
class Real_Time_Orders_and_Performance_Check__TestCases(unittest.TestCase):
        ...
        try:
            ...
            for exchange_dropdown_value in self.exchange_dropdown_values:
                ...
                for market_type in market_types:
                    ...
                    bt_ccxt_exchange = ut__construct_standalone_exchange(
    ...
    def test_10__strategy_less__long_position__open_and_close__market_order(self):
        start = timer()
        try:
                ...
                for symbol_id in symbols_id:
                    ...
                    if position.price == 0.0:
                        (nearest_ask, nearest_bid,
                         ) = instrument.get_orderbook_price_by_offset(0)

                        price = get_order_entry_price_and_queue(
                            position_type, nearest_ask, nearest_bid)
                        size = get_minimum_instrument_quantity(
                            price, instrument)
                        ...
                        entry_order = instrument.buy(**buy__dict)
                        _, entry_order_minutes, entry_order_seconds = get_time_diff(
                            entry_order_start)
                    ...
                    if position.price > 0.0:
                        ...
                        exit_order = instrument.sell(**sell__dict)
                        _, exit_order_minutes, exit_order_seconds = get_time_diff(
                            exit_order_start)
