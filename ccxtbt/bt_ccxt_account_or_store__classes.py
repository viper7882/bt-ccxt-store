...
class BT_CCXT_Account_or_Store(backtrader.with_metaclass(Meta_Account_or_Store, backtrader.Broker_or_Exchange_Base)):
                ...
                try:
                    ...
                    if isinstance(e, ExchangeError):
                        ...
                        if self.exchange_dropdown_value == BINANCE_EXCHANGE_ID:
                            # If the order is not found in the exchange
                            if exchange_error_dict['code'] == -1102:
                                '''
    ...
    def next(self, ut_provided__new_ccxt_order=None):
        ...
        for order in self.open_orders:
            ...
            if new_ccxt_order[self.parent.mappings[CCXT_ORDER_TYPES[OPENED_ORDER]]['key']] == \
                ...
                # But the backtrader status is not Accepted
    ...
    def fetch_closed_orders(self, symbol=None, since=None, limit=None, datafeed=None, params=None):
        if params is None:
            params = {}
        ccxt_closed_orders = self._fetch_closed_orders(
            symbol=symbol, since=since, limit=limit, params=params)
    ...
    def fetch_canceled_orders(self, symbol=None, since=None, limit=None, datafeed=None, params=None):
        if params is None:
            params = {}
        ccxt_orders = self._fetch_orders(
            symbol=symbol, since=since, limit=limit, params=params)
        ccxt_canceled_orders = \
            self.exchange.filter_by(ccxt_orders, DERIVED__CCXT_ORDER__KEYS[STATUS],
                                    self.parent.mappings[CCXT_ORDER_TYPES[CANCELED_ORDER]]['value'])

        handle_orders_routine__dict = dict(
            ccxt_orders=ccxt_canceled_orders,
    ...
    def fetch_rejected_orders(self, symbol=None, since=None, limit=None, datafeed=None, params=None):
        if params is None:
            params = {}
        ccxt_orders = self._fetch_orders(
            symbol=symbol, since=since, limit=limit, params=params)
        ccxt_rejected_orders = \
            self.exchange.filter_by(ccxt_orders, DERIVED__CCXT_ORDER__KEYS[STATUS],
                                    self.parent.mappings[CCXT_ORDER_TYPES[REJECTED_ORDER]]['value'])

        handle_orders_routine__dict = dict(
            ccxt_orders=ccxt_rejected_orders,
    ...
    def _post_process__after_child_is_added(self, instrument):
